...
const CImage = async ({txt, ctgs = [], imgPath, req}) => {
	...
	try {
		...
		const url = `http://localhost${process.env.PORT || 9000}${imgPath}`
		...
		await image
			.resize(1000, 1000)
			.composite(topMask, 0, 0, {mode: Jimp.BLEND_SOURCE_OVER, opacityDest: 1})
			.composite(bottomMask, 0, 544, {
				mode: Jimp.BLEND_SOURCE_OVER,
				opacityDest: 1,
			})
			.composite(logo, 60, 61, {mode: Jimp.BLEND_SOURCE_OVER, opacityDest: 1})
			.print(oldStandart, 60, 742, txt, 850, 216)
		...
		await image.writeAsync(path.resolve(__dirname, "../uploads/output/Futbol.png"))
	} catch (error) {
		console.log(error)
	}
}
